...
function taskFun(cb, filename) {
  return gulp.src(filename ? filename : ["Source/McsClass/**/*.ts"])
        ...
        .pipe(gulp.dest(filename ? path.dirname(filename) : "Source/McsClass/"))
  // return tsProject.src()
  // .pipe(tsProject())
  // .js.pipe(gulp.dest("Source/McsClass/"));
}
gulp.task('watchTsac',taskFun)
gulp.task('transTs',function() {
  return gulp.watch(["Source/McsClass/**/*.ts"], gulp.series("watchTsac"));
})

gulp.task('startTs',gulp.parallel('watchTsac','transTs'))
...
gulp.task(
  ...
  gulp.series("build", function () {
    return combineJavaScript({
      removePragmas: false,
      optimizer: "uglify2",
      outputDirectory: path.join("Build", "Mcs"),
    });
  })
);
...
gulp.task("test", function (done) {
  ...
  if (release) {
    files = [
      ...
      { pattern: "Specs/ThirdParty/**", included: true, type: "module" },
      { pattern: "Specs/TestWorkers/**", included: false },
      { pattern: "Build/Mcs/Mcs.js", included: true },
      { pattern: "Build/Mcs/**", included: false },
        ...
        for (let q = 0; q < sortedNames.length; q++) {
          ...
          if (file.startsWith("Specs")) {
            ...
            if (modulePath.startsWith("Specs") || modulePath.startsWith(".")) {
              ...
              modulePath =
                "'" + path.relative(sourceDir, "Source") + "/Mcs.js" + "'";
  ...
  return rollup
    ...
    .then(function (bundle) {
      return bundle.write({
        format: "umd",
        name: "Mcs",
        file: path.join(combineOutput, "Mcs.js"),
        sourcemap: debug,
...
function createCesiumJs() {
  ...
  fs.writeFileSync("Source/Mcs.js", contents);
}
    ...
    const stream = mergeStream(
      ...
      gulp.src(
        [
          "Build/Mcs/Assets/**",
          "Build/Mcs/Workers/**",
          "Build/Mcs/ThirdParty/**",
          "Build/Mcs/Widgets/**",
          "!Build/Mcs/Widgets/**/*.css",
        ],
        {
          base: "Build/Mcs",
          nodir: true,
      ...
      gulp.src(["Build/Mcs/Widgets/InfoBox/InfoBoxDescription.css"], {
        base: "Build/Mcs",
      }),
